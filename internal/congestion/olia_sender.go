...
type OliaSender struct {
	...
	olia            *Olia
	oliaSenders     map[uint32]*OliaSender
	...
	maxTCPCongestionWindow protocol.ByteCount
...
func NewOliaSender(oliaSenders map[uint32]*OliaSender, rttStats *RTTStats, initialCongestionWindow, initialMaxCongestionWindow protocol.ByteCount) SendAlgorithmWithDebugInfo {
	return &OliaSender{
		rttStats:                   rttStats,
...
func (o *OliaSender) TimeUntilSend(bytesInFlight protocol.ByteCount) time.Duration {
	if o.InRecovery() {
		// PRR is used when in recovery.
		if o.prr.CanSend(o.GetCongestionWindow(), bytesInFlight, o.GetSlowStartThreshold()) {
			return 0
		}
	}
...
func (o *OliaSender) OnPacketSent(sentTime time.Time, bytesInFlight protocol.ByteCount, packetNumber protocol.PacketNumber, bytes protocol.ByteCount, isRetransmittable bool)  {
...
func getMaxCwnd(m map[uint32]*OliaSender) protocol.ByteCount {
	var bestCwnd protocol.ByteCount
	for _, os := range m {
		bestCwnd = utils.MaxByteCount(bestCwnd, os.congestionWindow)
	}
	return bestCwnd
}

func getRate(m map[uint32]*OliaSender, pathRTT time.Duration) protocol.ByteCount {
	// We have to avoid a zero rate because it is used as a divisor
	var rate protocol.ByteCount = 1
	var tmpCwnd protocol.ByteCount
...
func (o *OliaSender) maybeIncreaseCwnd(ackedPacketNumber protocol.PacketNumber, ackedBytes protocol.ByteCount, bytesInFlight protocol.ByteCount) {
	...
	if o.InSlowStart() {
		...
		o.congestionWindow = utils.MinByteCount(o.maxTCPCongestionWindow, o.olia.CongestionWindowAfterAck(o.congestionWindow, rate, cwndScaled))
	}
}

func (o *OliaSender) OnPacketAcked(ackedPacketNumber protocol.PacketNumber, ackedBytes protocol.ByteCount, bytesInFlight protocol.ByteCount, eventTime time.Time) {
...
func (o *OliaSender) OnPacketLost(
		...
		if o.lastCutbackExitedSlowstart {
			...
			if o.slowStartLargeReduction {
				if o.stats.slowstartPacketsLost == 1 || (o.stats.slowstartBytesLost/protocol.DefaultTCPMSS) > (o.stats.slowstartBytesLost - lostBytes)/protocol.DefaultTCPMSS {
					...
					o.congestionWindow = utils.MaxByteCount(o.congestionWindow-1, o.minCongestionWindow)
				}
				o.slowstartThreshold = o.congestionWindow
			}
		}
	...
	if o.slowStartLargeReduction && o.InSlowStart() {
		...
		o.congestionWindow = protocol.ByteCount(float32(o.congestionWindow) * o.RenoBeta())
	}
...
func (o *OliaSender) SlowstartThreshold() protocol.ByteCount {
